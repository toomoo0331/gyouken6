...
var ThView = function(arg) {
	...
	this.firstview_y = (arg.firstview_y == undefined) ? 0 : this.d2r(-arg.firstview_y);// 0 .. 360 (0)
	...
	this.tilt = this.firstview_y;
	var limit = Math.PI / 2 - 0.1;
	if (this.tilt > limit) this.tilt = limit;
	if (this.tilt < -limit) this.tilt = -limit;
	this.cameraDir = new THREE.Vector3(Math.sin(this.pan) * Math.cos(this.tilt),
	 Math.sin(this.tilt), Math.cos(this.pan) * Math.cos(this.tilt));
	this.oldPosition = {x:null, y:null};//マウスの変化量を表すための値
...
ThView.prototype.getCameraDir = function() {
	return this.cameraDir;
}

ThView.prototype.getCameraZoom = function() {
	return this.zoom;
}
